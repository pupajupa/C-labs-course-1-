...
public:
	...
	DequeIterator<T> begin() {
		return DequeIterator<T>(a, capacity, front_);
	}
	DequeIterator<T> end() {
		return DequeIterator<T>(a, capacity, (back_+1) % capacity);
	}
...
void Deque<T>::resize(size_t newcapacity) {  //Функция изменения размера.
	...
	size_t i = 0;
	for (auto it = begin(); it != end(); ++it,i++) {     //Продвигаемся по деку, пока счетчик не будет равен позиции последнего элемента.
		*tmp[i]=*it;
	}
	for(size_t j = 0;j<capacity;j++){
		delete a[j];
	}  //Так как в цикле перенос последнего элемента из старого массива в новый не произошел, вручную переносим последний элемент старого массива.
